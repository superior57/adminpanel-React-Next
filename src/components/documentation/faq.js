...
export const Customization = () => {
  const questions = [
    ...
    },
    {
      ...
      answer: (
        <div>
          We provide an empty example navigation file in <code>src/navigation/empty.js</code>. Replace <code>src/navigation/index.js</code> with the content of this file to start developing your own navigation menus.
        </div>
      )
    },
    {
      question:
        'How can I customize the side navigation?',
      answer: (
        <div>
          Side navigation options can be found in the <code>src/store.js</code> file.
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      answer: (
        <div>
          Global layouts are defined in the <code>src/store.js</code> file. Also layouts
          can defined in every page by importing the layout directly from the <code>src/layouts</code> folder or by overriding the <code>redux</code> configuration. For examples go to the <code>src/pages/demo</code> folder.
        </div>
      )
    },
    {
      ...
      answer: (
        <div>
          ...
          <code>colors</code> section, in line 33. The same colors are copied to the <code>src/functions/colors.js</code> which makes them available for javascript components such as charts and maps.
        </div>
      )
    },
    {
      ...
      answer: (
        <div>
          Layouts colors are defined in the <code>src/scss/palettes</code> folder. In this folder you can find the palette definitions for the <code>background, left sidebar, logo, navbar, top navigation and right sidebar</code> components.
        </div>
      )
    },
    {
      ...
      answer: (
        <div>
          SCSS files are imported into the template by the <code>src/pages/_app.js</code> file. To add a new file just import it at the top of this file.
        </div>
      )
    },
    {
      question: 'How do I set the page background, left sidebar, navbar, logo, top navigation and right sidebar colors?',
      answer: (
        <div>
          Palette colors are defined in the <code>scss/_variables.scs</code> file and the color definitions for each palette are set in the <code>scss/palettes</code> and <code>scss/components</code> folders
        </div>
      )
    },
    {
      ...
      answer: (
        <div>
          ...
          <code>scss/components</code> folder and can either be set there globally or
          be modified using the <b>Tailwind SCSS</b> background, text and border
          color modifiers. All available modifiers are listed in the{' '}
          <code>http://localhost:3333/ui-elements/colors</code> page
        </div>
      )
    },
    {
